...
## Cross-Site Request Forgery (CSRF) Attacks and Tokens


Not everyone is a good person (shocker). 

You are logged into your bank’s website in one browser tab. In another tab, you visit a site with a hidden form that tries to 
...
<details>
  ...
  <div style={{ marginLeft: "20px" }}>
    ...
    By using a CSRF token — a unique, random value generated for each session and included in every form - the server checks that the token matches what it expects. 
